export default ({ input }: Input) => {
  ...
  // Check path position for loop opportunity
  const looping = path.filter(({ x, y }) => {
    area[y][x] = "#";
    const { loop } = getRoundDetails(_x, _y, _dir);
    area[y][x] = ".";
    return loop;
  });

  // Count the number of unique loop opportunities
  return new Set(looping.map(({ x, y }) => `${x},${y}`)).size;
};
